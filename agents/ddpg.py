...
from agents.networks import Actor, Critic, CriticSimpleCat
...
class DDPGAgentSimpleCatEnsemble(DDPGAgentEnsemble):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.critic = CriticSimpleCat(
            self.state_size, self.action_size, self.critic_fc_layers
        ).to(device)
        self.critic_target = CriticSimpleCat(
            self.state_size, self.action_size, self.critic_fc_layers
        ).to(device)
        self.critic_optimizer = Adam(self.critic.parameters(), lr=self.critic_lr)
